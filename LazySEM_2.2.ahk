...
if RegexMatch(Clipboard, "`{[\d-+]+:[\d-+]+`}")>0
	...
	ReplaceList := Object()
	StringReplace, Temp, Clipboard, `r`n, A_Space, All
	Words := RegExReplace(Temp, ".*`{(.+)`}.*", "$1")
	
	Loop, parse, Words, %A_Space%
		{
		if A_Loopfield = ""
			Continue
		if !hasValue(ReplaceList, A_Loopfield)
			ReplaceList.Insert(A_Loopfield)
		}
	
	Temp:=RegExReplace(Clipboard, "`{.+\s.+`}", "`{R`}")
	
	For index, value in ReplaceList
		{
		StringReplace, AddThis, Temp,`{R`}, %value% , All
		Outstring:=OutString . AddThis . "`r`n"
		}
	Clipboard := Outstring
	send ^v
...
Loop, parse, Text, `n, `r
	...
	if Sections0 != 2
		{
		MsgBox Please select two columns of MPlus results!
		Return
		}
